                ...
                <div class="form-group mb-3">
                    ...
                    <select name="bank_id" class="form-control">
                        ...
                        @foreach ($bank as $value)
                            <option value="{{ $value->id }}" @if($rekening->bank_id==$value->id) {{ 'selected' }} @endif>{{ $value->name_bank }}</option>
                        @endforeach
                    </select>
                    <i class="text-danger">{{ $errors->first('bank_id') }}</i>
                </div>
                <div class="form-group mb-3">
                    <label for="" class="form-label">Name Rekening</label>
                    <input type="text" name="name_rekening" class="form-control" value="{{ $rekening->name_rekening }}">
