                ...
                <div class="card-body">
                    <div class="d-sm-flex d-block align-items-center justify-content-between mb-9">
                        <div class="mb-3 mb-sm-0">
                            <h5 class="card-title fw-semibold">Order di bulan {{ date('M') }}</h5>
                        </div>
                    </div>
                    <div id="chart1"></div>
                </div>
...
@section('script')
    <script>
        var max = '{{ $order->count() }}';
        var keranjang = '{{ $order_keranjang->count() }}';
        var pembayaran = '{{ $order_pembayaran->count() }}';
        var selesai = '{{ $order_selesai->count() }}';
        var chart = {
            series: [{
                    name: 'Order',
                    data: [keranjang, pembayaran, selesai]
                },
            ],
            ...
            xaxis: {
                type: "category",
                categories: ["Keranjang", "Pembayaran", "Selesai"],
                labels: {
                    style: {
